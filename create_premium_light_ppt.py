"""
Q2 2026 Professional Work Summary — Premium Light Theme
"""
...
# ── Theme Colors ──────────────────────────────────────────────
BG_LIGHT   = RGBColor(248, 250, 252) # Slate 50 (Off-white background)
CARD_LIGHT = RGBColor(255, 255, 255) # Pure White
TECH_BLUE  = RGBColor(0, 102, 255)   # Vibrant Tech Blue #0066FF
AMBER      = RGBColor(245, 158, 11)  # #F59E0B
TEXT_MAIN  = RGBColor(15, 23, 42)    # Slate 900 #0F172A
TEXT_SUB   = RGBColor(71, 85, 105)   # Slate 600 #475569
BORDER_GRY = RGBColor(203, 213, 225) # Slate 300 #CBD5E1
...
def add_textbox(slide, left, top, width, height, text, size=18, color=TEXT_MAIN, bold=False, align=PP_ALIGN.LEFT):
...
def add_bullet_list(slide, items, left, top, width, height, size=12, color=TEXT_SUB):
...
def slide_template(slide, title, subtitle):
    # Background
    add_shape(slide, MSO_SHAPE.RECTANGLE, 0, 0, 13.333, 7.5, BG_LIGHT)
    
    # Header Graphics
    add_shape(slide, MSO_SHAPE.RECTANGLE, 0.5, 0.4, 0.1, 0.6, TECH_BLUE)
    
    # Title & Subtitle
    add_textbox(slide, 0.8, 0.35, 12, 0.6, title, size=28, color=TEXT_MAIN, bold=True)
    if subtitle:
        add_textbox(slide, 0.8, 0.85, 12, 0.4, subtitle, size=14, color=TECH_BLUE)
        
    # Footer
    add_shape(slide, MSO_SHAPE.RECTANGLE, 0.5, 7.2, 12.333, 0.02, BORDER_GRY)
    add_textbox(slide, 0.5, 7.22, 12, 0.3, "myG TECHNOLOGY | ENGINEERING & AI | Q2 2026", size=9, color=TEXT_SUB, align=PP_ALIGN.LEFT)

# --- SLIDE 1: TITLE ---
s1 = prs.slides.add_slide(BLANK)
add_shape(s1, MSO_SHAPE.RECTANGLE, 0, 0, 13.333, 7.5, BG_LIGHT)
# Grid accent
add_shape(s1, MSO_SHAPE.RECTANGLE, 12.0, 0, 1.333, 7.5, CARD_LIGHT)
add_shape(s1, MSO_SHAPE.RECTANGLE, 12.0, 1.5, 0.05, 4.5, TECH_BLUE)
add_shape(s1, MSO_SHAPE.RECTANGLE, 12.0, 1.5, 1.333, 0.05, TECH_BLUE)

add_textbox(s1, 1.0, 2.0, 10.0, 0.5, "QUARTERLY BUSINESS REVIEW", size=14, color=AMBER, bold=True)
add_textbox(s1, 1.0, 2.5, 10.0, 1.5, "Q2 2026 WORK\nSUMMARY", size=60, color=TEXT_MAIN, bold=True)
add_shape(s1, MSO_SHAPE.RECTANGLE, 1.0, 4.8, 3.5, 0.05, TECH_BLUE)
add_textbox(s1, 1.0, 5.2, 10.0, 0.8, "Jasil N Balussery", size=24, color=TEXT_MAIN, bold=True)
add_textbox(s1, 1.0, 5.7, 10.0, 0.8, "Technology / Software Development | myG", size=16, color=TEXT_SUB)

# --- SLIDE 2: EXEC SUMMARY ---
s2 = prs.slides.add_slide(BLANK)
slide_template(s2, "EXECUTIVE SUMMARY", "HIGH-LEVEL KPIS & DELIVERY METRICS")

add_textbox(s2, 0.5, 1.6, 12.3, 1.0, "During Q2 2026, extensive full-stack development, database engineering, AI/ML integration, and multiple greenfield product launches were delivered across 8 distinct projects. Work spanned Python (Flask/Django), PostgreSQL, JavaScript, Machine Learning, LLM integration, Google APIs, and cloud deployment.", size=16, color=TEXT_SUB)
...
for i, (val, lbl) in enumerate(metrics):
    ...
    lx = 0.5 + col * 4.2
    ty = 3.0 + row * 1.8
    add_shape(s2, MSO_SHAPE.ROUNDED_RECTANGLE, lx, ty, 3.8, 1.5, CARD_LIGHT, BORDER_GRY)
    add_shape(s2, MSO_SHAPE.RECTANGLE, lx + 0.2, ty + 0.3, 0.05, 0.9, TECH_BLUE)
    add_textbox(s2, lx + 0.4, ty + 0.35, 3.0, 0.6, val, size=36, color=TEXT_MAIN, bold=True)
    add_textbox(s2, lx + 0.4, ty + 0.9, 3.0, 0.5, lbl, size=14, color=AMBER, bold=True)

# --- HELPER FOR CONTENT SLIDES ---
def add_card(slide, left, top, width, height, title, points):
    add_shape(slide, MSO_SHAPE.ROUNDED_RECTANGLE, left, top, width, height, CARD_LIGHT, BORDER_GRY)
    add_shape(slide, MSO_SHAPE.RECTANGLE, left + 0.3, top + 0.45, 0.4, 0.05, TECH_BLUE) 
    add_textbox(slide, left + 0.3, top + 0.1, width - 0.4, 0.4, title, size=18, color=TEXT_MAIN, bold=True)
...
add_shape(s8, MSO_SHAPE.RECTANGLE, 0.5, 1.8, 12.33, 0.4, TECH_BLUE)
add_textbox(s8, 0.6, 1.8, 3.0, 0.4, "PROJECT", size=12, color=BG_LIGHT, bold=True)
add_textbox(s8, 4.0, 1.8, 5.0, 0.4, "DESCRIPTION", size=12, color=BG_LIGHT, bold=True)
add_textbox(s8, 9.5, 1.8, 2.0, 0.4, "PLATFORM", size=12, color=BG_LIGHT, bold=True)

for idx, (p_name, p_desc, p_plat) in enumerate(projects):
    ty = 2.3 + idx * 0.55
    bg_color = CARD_LIGHT if idx % 2 == 0 else BG_LIGHT
    add_shape(s8, MSO_SHAPE.RECTANGLE, 0.5, ty, 12.33, 0.5, bg_color, BORDER_GRY)
    add_textbox(s8, 0.6, ty + 0.1, 3.3, 0.4, p_name, size=13, color=TEXT_MAIN, bold=True)
    add_textbox(s8, 4.0, ty + 0.1, 5.3, 0.4, p_desc, size=13, color=TEXT_SUB)
    add_textbox(s8, 9.5, ty + 0.1, 2.0, 0.4, p_plat, size=13, color=AMBER, bold=True)

# --- SLIDE 9: THANK YOU ---
s9 = prs.slides.add_slide(BLANK)
add_shape(s9, MSO_SHAPE.RECTANGLE, 0, 0, 13.333, 7.5, BG_LIGHT)
# Grid accent
add_shape(s9, MSO_SHAPE.RECTANGLE, 0, 0, 1.333, 7.5, CARD_LIGHT)
add_shape(s9, MSO_SHAPE.RECTANGLE, 1.283, 1.5, 0.05, 4.5, TECH_BLUE)
add_shape(s9, MSO_SHAPE.RECTANGLE, 0, 1.5, 1.333, 0.05, TECH_BLUE)

add_textbox(s9, 0, 3.0, 13.333, 1.0, "THANK YOU", size=54, color=TEXT_MAIN, bold=True, align=PP_ALIGN.CENTER)
add_shape(s9, MSO_SHAPE.RECTANGLE, 6.166, 4.2, 1.0, 0.05, TECH_BLUE)
add_textbox(s9, 0, 4.5, 13.333, 0.5, "Jasil N Balussery | Technology / Software Development", size=18, color=TEXT_SUB, align=PP_ALIGN.CENTER)
add_textbox(s9, 0, 5.0, 13.333, 0.5, "April | May | June 2026", size=14, color=AMBER, bold=True, align=PP_ALIGN.CENTER)

OUTPUT = r"c:\Users\jasil_myg\Desktop\OSG-myG-PORTAL-mainnnnn - Copy\Q2_2026_Work_Summary_Premium_Light.pptx"
prs.save(OUTPUT)
